...
tinymce.PluginManager.add('previewAdv', (editor, url) => {
  ...
  editor.on('init', function () {
    ...
    // === TIMER ===
    let typingTimer
    const typingDebounce = 1000

    // === EDITOR ACTIONS ===
    editor.on('keydown', function () { // Writing...
      clearTimeout(typingTimer)
    })

    editor.on('keyup', function () { // Stop writing, wait `const typingDebounce` second and insert the adv
      clearTimeout(typingTimer)
      typingTimer = setTimeout(insertAdvAndRepositionCursor, typingDebounce)
    })
  })
   ...
   */
  function insertAdvAndRepositionCursor () {
    insertAdv()
