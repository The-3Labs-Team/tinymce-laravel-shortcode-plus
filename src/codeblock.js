...
tinymce.PluginManager.add('codeblock', function (editor, url) {
  ...
  const isCodeNode = function (node) {
    if (!node) {
      return false
    }
    ...
    return String(nodeName).toUpperCase() === 'CODE'
  }

  const getSelectedCodeNode = function () {
    const selectedNode = typeof editor.selection.getNode === 'function'
    ...
    if (editor.dom && typeof editor.dom.getParent === 'function') {
      return editor.dom.getParent(selectedNode, 'code')
    }

    return null
  }

  const removeCodeBlock = function (codeNode) {
    if (!codeNode || !editor.dom || typeof editor.dom.remove !== 'function') {
      return
    }
  ...
  const handleAction = function () {
    const selectedCodeNode = getSelectedCodeNode()

    if (selectedCodeNode) {
      removeCodeBlock(selectedCodeNode)
      return
    }
  ...
  editor.ui.registry.addToggleButton('codeblock', {
    ...
    onSetup: function (api) {
      if (!editor.formatter || typeof editor.formatter.formatChanged !== 'function') {
        return function () {}
      }

      const formatChanged = editor.formatter.formatChanged('code', function (state) {
        api.setActive(state)
      })

      return function () {
        if (formatChanged && typeof formatChanged.unbind === 'function') {
